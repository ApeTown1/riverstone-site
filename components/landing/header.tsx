        ...
        <header className="border-b border-white/20 bg-white/80 backdrop-blur-2xl fixed top-0 left-0 w-full z-50 shadow-sm">
            <div className="container mx-auto px-4 lg:px-6 h-20 flex items-center justify-between">
                ...
                <nav ref={navRef} className={`items-center space-x-10 ${forceMobile ? 'hidden' : 'flex'}`}>
                    ...
                    <div className="relative group" onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
                        ...
                        <div className={`absolute left-0 mt-2 w-56 bg-white border border-neutral-200 rounded-xl shadow-lg transition-opacity duration-200 z-50 ${dropdownOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                            <ul className="py-2">
                                <li><a href="#case-studies" data-casestudy="0" className="block px-6 py-3 text-neutral-700 hover:bg-neutral-100 rounded-xl transition-all duration-200">Go-to-Market & Ecosystem Launch</a></li>
                                <li><a href="#case-studies" data-casestudy="1" className="block px-6 py-3 text-neutral-700 hover:bg-neutral-100 rounded-xl transition-all duration-200">Financial Modeling & Valuation</a></li>
                                <li><a href="#case-studies" data-casestudy="2" className="block px-6 py-3 text-neutral-700 hover:bg-neutral-100 rounded-xl transition-all duration-200">Fundraising & Investor Strategy</a></li>
                            </ul>
                        </div>
                    </div>
                ...
                >
                    <button
                        ref={burgerButtonRef}
                        className={`p-2 rounded-xl border border-neutral-300 bg-white/80 hover:bg-neutral-100 focus:outline-none transition-transform duration-200 ${mobileMenuOpen ? 'scale-125' : 'scale-100'}`}
                    ...
                    {mobileMenuOpen && (
                        ...
                        >
                            <Link href="/about" className="text-lg font-semibold text-neutral-700 hover:text-neutral-900" onClick={handleMenuClose}>About</Link>
                            <Link href="/#services" className="text-lg font-semibold text-neutral-700 hover:text-neutral-900" onClick={handleMenuClose}>Services</Link>
                            <Link href="/#insights" className="text-lg font-semibold text-neutral-700 hover:text-neutral-900" onClick={handleMenuClose}>Market Insights</Link>
                            <Link href="/#case-studies" className="text-lg font-semibold text-neutral-700 hover:text-neutral-900" onClick={handleMenuClose}>Case Studies</Link>
                            <Link href="/contact" className="text-lg font-semibold text-neutral-700 hover:text-neutral-900" onClick={handleMenuClose}>Contact</Link>
                        </div>
                    )}
                </div>
            </div>
